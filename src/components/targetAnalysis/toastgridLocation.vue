...
import { priceToString, isMobile, uuidv4, getMedia, getLocation } from "@/assets/js/common.js";
...
export default {
  ...
  data() {
    return {
      ...
      columns: [],
      data: [],
      locations: ["00","01","02","03","04","05","06","07","08","09","10","11","12","13","14","15","16","17","99","-1"],
      grid: null
    };
  },
  computed: {
  ...
  methods: {
    ...
    initToastGrid() {
      var vm = this;
      var complexColumns = vm.locations.map(location => ({ header: `${vm.getLocation(location)}`, name: `mergeColumn${location}`, childNames: [`im${location}`,`clk${location}`,`cst${location}`] }));
      ...
      var hourColumns = vm.locations.flatMap(location => ([ { header: '노출수', name: `im${location}`, width: 100, align: 'center' }, 
      { header: '클릭수', name: `clk${location}`, width: 100, align: 'center' }, { header: '광고비', name: `cst${location}`, width: 100, align: 'center' } ]));
    ...
    getLocation(location) { return getLocation(location);},
