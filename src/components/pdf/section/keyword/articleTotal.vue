<template>
  <div :class="getUniqueId">
    <template v-for="(index, i) in main.keywordAnalysis.total[0].total.pdf.end" :key="index + i + getUuidv4()">
        ...
        <section class="d-flex pdf-card pdf-card-2 naver">
            <div class="report" v-if="main.keywordAnalysis.responseData !== null">
              <table class="report">
                ...
                <tbody>
                  <template v-for="(d, i) in getDatasForKeywordTotal('total', 0, index)" :key="d + i + getUuidv4()">
                    <tr >
...
export default {
  props: { keywordAnalysis: Object, pReport: Object, pDropdownMenu: Object, },
  data() {
    return {
  ...
  methods: {
    ...
    setKeywordAnalysis() {
      ...
      vm.main.keywordAnalysis.pages[2].google.responseData = {};
    },    
    getDatasForKeyword(media, arrindex, index) {
      ...
      if (index === 1) {
        ...
        start = index - 2 > 0 ? vm.main.keywordAnalysis.pages[arrindex][media].pdf.spare + vm.main.keywordAnalysis.pages[arrindex][media].pdf.d * (index - 2) : vm.main.keywordAnalysis.pages[arrindex][media].pdf.spare;
      }
    ...
    getDatasForKeywordTotal(media, arrindex, index) {
      ...
      if (index === 1) {
        ...
        start = index - 2 > 0 ? vm.main.keywordAnalysis.total[arrindex][media].pdf.spare + vm.main.keywordAnalysis.total[arrindex][media].pdf.d * (index - 2) : vm.main.keywordAnalysis.total[arrindex][media].pdf.spare;
      }
      ...
      $.ajax({
        ...
        success: function (data) {
          if(data.result === "success") {
            ["total"].forEach(function (media, index) {
              if (vm.report.activePage.keywordAnalysis) {
                if (vm.isOneMedia()) {
                  vm.$emit('rendering', {graph: 'keywordreportTotalForKeyword', status: true});
                } else {
                  vm.main.keywordAnalysis.total[index][media].responseData = data;
                  vm.main.keywordAnalysis.total[index][media].report = [];
                  Object.keys(data.data.keywords).forEach( function (keyword, index2) {
                      var obj = data.data.keywords[keyword];
                      vm.main.keywordAnalysis.total[index][media].report.push(obj);
                    }
                  );

                  var length = vm.main.keywordAnalysis.total[index][media].report.length - vm.main.keywordAnalysis.total[index][media].pdf.spare;
                  vm.main.keywordAnalysis.total[index][media].pdf.end = Math.floor( length / vm.main.keywordAnalysis.total[index][media].pdf.d );
                  var n = length % vm.main.keywordAnalysis.total[index][media].pdf.d;

                  if (vm.main.keywordAnalysis.total[index][media].pdf.end <= 0)
                    vm.main.keywordAnalysis.total[index][media].pdf.end += 1;
                  else if (n > 0) vm.main.keywordAnalysis.total[index][media].pdf.end += 1;

                  // vm.main.keywordAnalysis.total[index][media].pdf.end += 1; //12개용 페이지 추가
                  vm.$emit("finished", {graph: "keywordreportTotalForKeyword", status: true});
                }
              }
            });
          }
