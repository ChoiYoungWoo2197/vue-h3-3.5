<template>
  <div :class="getUniqueId" class="pdf-a4-l mediaAnalysis">
    <section class="d-flex pdf-card pdf-card-1">
      <div class="sub-title">
        <h2>{{ "매체별 리포트" + " - " }}
          <span v-if="report.activeMedia.kakaomo && report.activeMedia.naverda">{{"배너광고(모먼트, GFA)"}}</span>
          <span v-else-if="report.activeMedia.kakaomo">{{"카카오 모먼트"}}</span>
          <span v-else-if="report.activeMedia.naverda">{{"GFA"}}</span>
        </h2>
      </div>
    ...
    <template v-if="report.activeMedia.kakaomo">
      <section class="d-flex pdf-card pdf-card-2 kakaomo">
        <div class="row justify-content-between" ref="graphs">
        ...
        <div class="report" v-if="main.mediaAnalysis.responseData !== null">
          <table>
            ...
            <tbody>
              <tr class="subdata">
                <td class="text-left">카카오 모먼트</td>
                <template v-for="(k,i) in Object.keys(sortDraggable())">
                  <td :key="k + i + getUuidv4()" v-if="report.activeBtn[k]">{{priceToString(main.mediaAnalysis.datas.dsp.kakaomo[k]) + getUnits(k)}}</td>
                </template>
              </tr>
            </tbody>
          </table>
    ...
    <template v-if="report.activeMedia.naverda">
      <section class="d-flex pdf-card pdf-card-2 naverda">
        ...
        <div class="report" v-if="main.mediaAnalysis.responseData !== null">
          <table>
            ...
            <tbody>
              <tr class="subdata">
                <td class="text-left">GFA</td>
                <template v-for="(k,i) in Object.keys(sortDraggable())">
                  <td :key="k + i + getUuidv4()" v-if="report.activeBtn[k]">{{priceToString(main.mediaAnalysis.datas.dsp.naverda[k]) + getUnits(k)}}</td>
                </template>                
              </tr>
            </tbody>
          </table>
...
export default {
  ...
  created() {
    // this.report.activeMedia.kakaomo = false;
    // this.report.activeMedia.naverda = true;
                ...
                Object.keys(vm.main.mediaAnalysis.pages[index][page].graph).forEach(
                  function (graph, index2) {
                    ...
                    vm.$nextTick(function () {
                      // console.log(vm.$refs, document.querySelector(".mediaAnalysis " + " ." + page + " #" + graph));
                      if(document.querySelector(".mediaAnalysis " + " ." + page + " #" + graph)) {
                      ...
                      vm.main.mediaAnalysis.pages[index][page].graph[graph].setOption(option);
                      vm.main.mediaAnalysis.pages[index][page].graph[graph].on("finished", function () {
