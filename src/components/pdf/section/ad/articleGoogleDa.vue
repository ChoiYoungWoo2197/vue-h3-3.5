<template>
  <div :class="getUniqueId">
    <template v-for="(index, i) in main.adAnalysis.pages[3].google.pdf.end" :key="index + i + getUuidv4()">
      <div  class="pdf-a4-l adAnalysis">
        <section class="d-flex pdf-card pdf-card-1">
            <div class="sub-title"> <h2>소재별 분석 - 구글</h2> </div>
        ...
        <section class="d-flex pdf-card pdf-card-2 google">
          <div class="report banner">
            <table class="report"  v-if="main.adAnalysis.pages[3].responseData !== null">
                <thead>
                  <tr>
                ...
                <tbody>
                  <template v-for="(d, i) in getDatasForAd('google', 3, index)" :key="d + i + getUuidv4()">
                    <tr >
...
export default {
  ...
  methods: {
    ...
    setAdAnalysis() {
      ...
      vm.main.adAnalysis.pages[3].google.responseData = {};
    },  
    getReport() {
      let vm = this;
      $.ajax({
        ...
        success: function (data) {
          if(data.result === "success") {
            ["naver", "kakaosa", "kakaomo","google"].forEach(function (media, index) {
              ...
              if (vm.report.activePage.adAnalysis && vm.report.activeMedia[media]) {
                ...
                var length = vm.main.adAnalysis.pages[index][media].report.length - vm.main.adAnalysis.pages[index][media].pdf.spare;
                vm.main.adAnalysis.pages[index][media].pdf.end = Math.floor( length / vm.main.adAnalysis.pages[index][media].pdf.d );
                ...
                vm.$emit("finished", {graph: "adreportgoogledaForAdToBanner", status: true});
              }
            });
          }
        },
