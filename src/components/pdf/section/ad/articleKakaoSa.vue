<template>
  <div :class="getUniqueId">
    <template v-for="(index, i) in main.adAnalysis.pages[1].kakaosa.pdf.end" :key="index + i + getUuidv4()">
      <div  class="pdf-a4-l adAnalysis">
        <section class="d-flex pdf-card pdf-card-1">
            <div class="sub-title">
              <h2>소재별 분석 - 카카오 검색광고</h2>
            </div>
        ...
        <section class="d-flex pdf-card pdf-card-2 kakaosa">
            <div class="report">
              <table class="report"  v-if="main.adAnalysis.pages[1].responseData !== null">
                <thead>
                  <tr>
                ...
                <tbody>
                  <template v-for="(d, i) in getDatasForAd('kakaosa', 1, index)" :key="d + i + getUuidv4()">
                    <tr >
          ...
          if(data.result === "success") {
            ["naver", "kakaosa"].forEach(function (media, index) {
              ...
              if (vm.report.activePage.adAnalysis && vm.report.activeMedia[media]) {
                ...
                vm.$emit("finished", {graph: "adreportkakaosaForAdToSearch", status: true});
              }
            });
          }
        },
