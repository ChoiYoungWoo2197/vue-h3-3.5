<template>
  <div :class="getUniqueId">
    <template v-for="(index, i) in main.campaignAnalysis.pages[0].naver.pdf.end" :key="index + i + '-' + getUuidv4()">
      <div  class="pdf-a4-l campaignAnalysis">
        <section class="d-flex pdf-card pdf-card-1">
          <div class="sub-title">
            <h2>캠페인그룹분석 - 네이버 검색광고</h2>
          </div>
          ...
          <div class="report">
            <table class="report">
              ...
              <tbody v-if="main.campaignAnalysis.responseData !== null">
                <tr
                  v-for="(data, index2) in getDatasForCampaign('naver', 0, index)"
                  :key="data.cst + '-' + index2"
                  :class="
                    data.isMainData === true
                      ? ['maindata', 'collapsed']
                      : ['subdata', 'show', 'collapse', data.campain]
                  "
                ...
                >
                  <template v-if="data.isMainData === true">
                    <td
                      :class="
                        data.haschild === true ? ['text-left fold-btn'] : ['text-left']
                      "
                    >
                      {{ data.campaign_name }}
                      <!-- <a data-toggle="collapse" :href="'.' + data.campain" role="button" aria-expanded="false" :aria-controls="data.campain" class="collapsed" v-text="data.campaign_name"></a> -->
                    </td>
                  </template>
                  <template v-else>
                    <!-- <td></td> -->
                    <td class="text-left">
                      {{ data.campaign_name }}
                      <!-- <a class="btn" data-toggle="modal" data-target="#keyword-modal" @click="clickCampaignAnalysisModal(data.campaign_id)"><i class="bi bi-box-arrow-up-right"></i></a> -->
                    </td>
                  </template>
                  <template v-for="(k,i) in Object.keys(sortDraggable())">
                ...
                <template v-if="index === main.campaignAnalysis.pages[0]['naver'].pdf.end">
                  <tr
                    v-for="data in main.campaignAnalysis.pages[0].naver.total"
                    :key="data.cst + 'L'"
                    class="total bg-white"
                  >
  ...
  methods: {
    ...
    getTotalForCampaignAnalysis(data, type, media, index) {
      ...
      categorys.forEach(function (category, index) {
        ...
        datas.forEach(function (data, index2) {
          ...
          total.ctr = parseFloat(
            total.clk > 0 && total.im > 0 ? (total.clk / total.im) * 100 : 0
          ).toFixed(2);
          total.cpc = parseFloat(
            total.cst > 0 && total.clk > 0 ? total.cst / total.clk : 0
          ).toFixed(0);
          total.cpa = parseFloat(
            total.cst > 0 && total.cv > 0 ? total.cst / total.cv : 0
          ).toFixed(0);
          total.cvr = parseFloat(
            total.cv > 0 && total.clk > 0 ? (total.cv / total.clk) * 100 : 0
          ).toFixed(2);
          total.roas = parseFloat(
            total.cr > 0 && total.cst > 0 ? (total.cr / total.cst) * 100 : 0
          ).toFixed(0);
    ...
    getDatasForCampaign(media, arrindex, index) {
      ...
      if (index === 1) {
        ...
        start =
          index - 2 > 0
            ? vm.main.campaignAnalysis.pages[arrindex][media].pdf.spare +
              vm.main.campaignAnalysis.pages[arrindex][media].pdf.d * (index - 2)
            : vm.main.campaignAnalysis.pages[arrindex][media].pdf.spare;
      }
    ...
    getDatasForCampaignTotal(index) {
      ...
      if (index === 1) {
        ...
        start =
          index - 2 > 0
            ? vm.main.campaignAnalysis.total[0].total.pdf.spare +
              vm.main.campaignAnalysis.total[0].total.pdf.d * (index - 2)
            : vm.main.campaignAnalysis.total[0].total.pdf.spare;
      }
          ...
          if (data.result === "success") {
            ...
            vm.main.medias.forEach(function (media, index) { 
              if (vm.report.activePage.campaignAnalysis && vm.report.activeMedia[media]) { 
                ...
                var length =
                  vm.main.campaignAnalysis.pages[index][media].report.length -
                  vm.main.campaignAnalysis.pages[index][media].pdf.spare;
                vm.main.campaignAnalysis.pages[index][media].pdf.end = Math.floor(
                  length / vm.main.campaignAnalysis.pages[index][media].pdf.d
                );
                  ...
                  function (graph, index2) {
                    ...
                    vm.$nextTick(function () {
                      ...
                      if(document.querySelector(".campaignAnalysis " + " ." + media + " #" + graph)) {
                        ...
                        vm.main.campaignAnalysis.pages[index][media].graph[graph].on("finished", function () { 
                          vm.$emit("finished", {graph: graph + media, status: true});
                        });
                      }
                    });
                  }
