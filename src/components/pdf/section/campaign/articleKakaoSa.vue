<template>
  <div :class="getUniqueId">
    <template v-for="(index, i) in main.campaignAnalysis.pages[1].kakaosa.pdf.end" :key="index + i + '-' + getUuidv4()">
      <div  class="pdf-a4-l campaignAnalysis">
        <section class="d-flex pdf-card pdf-card-1">
          <div class="sub-title">
            <h2>캠페인그룹분석 - 카카오 검색광고</h2>
          </div>
          ...
          <div class="report">
            <table class="report">
              ...
              <tbody v-if="main.campaignAnalysis.responseData !== null">
                <tr
                  v-for="(data, index2) in getDatasForCampaign('kakaosa', 1, index)"
                  :key="data.cst + '-' + index2"
                  :class="
                ...
                <template v-if="index === main.campaignAnalysis.pages[1]['kakaosa'].pdf.end">
                  <tr
                    v-for="data in main.campaignAnalysis.pages[1].kakaosa.total"
                    :key="data.cst + 'L'"
