<template>
  <div :class="getUniqueId">
    <template v-for="(index, i) in main.periodAnalysis.total[1].dayweek.total.pdf.end" :key="index + i + '-' + getUuidv4()">
      <div  class="pdf-a4-l periodAnalysis">
        ...
        <section class="d-flex pdf-card pdf-card-2 naver">
          <div v-show="index === 1" class="row justify-content-between">
            <div class="col crossGraphPeriodTotalOnedayweek d-flex justify-content-center">
              <div
                id="crossGraphPeriodTotalOnedayweek"
                class="crossGraphPeriodTotalOnedayweek"
              ></div>
            </div>
            <div class="col crossGraphPeriodTotalTwodayweek d-flex justify-content-center">
              <div
                id="crossGraphPeriodTotalTwodayweek"
                class="crossGraphPeriodTotalTwodayweek"
              ></div>
            </div>
          </div>
          <div class="report">
            <table class="report">
              ...
              <tbody v-if="main.periodAnalysis.responseData !== null">
                <tr
                  v-for="(data, index2) in getDatasForPeriodTotal(1, 'dayweek', index)"
  ...
  methods: {
    ...
    getReport() {
      ...
      if (vm.report.activePage.periodAnalysis) {
        ...
        $.when(periodreportForDayweek).done(function(periodreportForDayweek) {
