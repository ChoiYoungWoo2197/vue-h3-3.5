<template>
  <div :class="getUniqueId">
    <template v-for="(index, i) in main.periodAnalysis.pages[3].naverda.hour.pdf.end" :key="index + i + '-' + getUuidv4()">
      <div  class="pdf-a4-l periodAnalysis">
        <section class="d-flex pdf-card pdf-card-1">
          <div class="sub-title">
            <h2>기간별 분석 - GFA(시간별)</h2>
          </div>
        ...
        <section class="d-flex pdf-card pdf-card-2 naverda">
          <div v-show="index === 1" class="row justify-content-between" ref="graphs">
            <div class="col crossGraphPeriodOnehour d-flex justify-content-center">
              <div
                id="crossGraphPeriodOnehour"
                class="crossGraphPeriodOnehour"
              ></div>
            </div>
            <div class="col crossGraphPeriodTwohour d-flex justify-content-center">
              <div
                id="crossGraphPeriodTwohour"
                class="crossGraphPeriodTwohour"
              ></div>
            </div>
          </div>
          <div class="report">
            <table class="report">
              ...
              <tbody v-if="main.periodAnalysis.responseData !== null">
                <tr
                  v-for="(data, index2) in getDatasForPeriod('naverda', 3, 'hour', index)"
      ...
      if (vm.report.activePage.periodAnalysis) {
        ...
        .done(function(periodreportForHour) {
          if(periodreportForHour.result === "success") {
            var isData = false;
            vm.main.periodAnalysis.pages[3].naverda.hour.responseData = periodreportForHour;
            vm.main.medias.forEach(function (media, index) {
              if (
                ...
                var graphDatas;
                var totalDatas = {
                  media: { kakaomo: {}, naverda: {}, kakaosa: {}, naver: {} },
                  total: { summary: {} },
                };
                ...
                var length =
                  vm.main.periodAnalysis.pages[index][media].hour.report.length -
                  vm.main.periodAnalysis.pages[index][media].hour.pdf.spare;
                vm.main.periodAnalysis.pages[index][media].hour.pdf.end = Math.floor(
                  length / vm.main.periodAnalysis.pages[index][media].hour.pdf.d
                );
              ...
              if (vm.isOneMedia()) {
                ...
                ["ctr", "cpc", "cpa", "cvr", "roas"].forEach(function (kpi, index2) {
                  for (i = 0; i <= 23; i++) {
                    ...
                    if (kpi === "ctr") {
                      obj2[kpi][hour] =
                        obj2["clk"][hour] > 0 && obj2["im"][hour] > 0
                          ? ((obj2["clk"][hour] / obj2["im"][hour]) * 100).toFixed(2)
                          : 0;
                    }
                    if (kpi === "cpc") {
                      obj2[kpi][hour] =
                        obj2["cst"][hour] > 0 && obj2["clk"][hour] > 0
                          ? (obj2["cst"][hour] / obj2["clk"][hour]).toFixed(0)
                          : 0;
                    }
                    if (kpi === "cpa") {
                      obj2[kpi][hour] =
                        obj2["cst"][hour] > 0 && obj2["cv"][hour] > 0
                          ? (obj2["cst"][hour] / obj2["cv"][hour]).toFixed(0)
                          : 0;
                    }
                    if (kpi === "cvr") {
                      obj2[kpi][hour] =
                        obj2["cv"][hour] > 0 && obj2["clk"][hour] > 0
                          ? ((obj2["cv"][hour] / obj2["clk"][hour]) * 100).toFixed(2)
                          : 0;
                    }
                    if (kpi === "roas") {
                      obj2[kpi][hour] =
                        obj2["cr"][hour] > 0 && obj2["cst"][hour] > 0
                          ? ((obj2["cr"][hour] / obj2["cst"][hour]) * 100).toFixed(0)
                          : 0;
                    }
                ...
                var length =
                  vm.main.periodAnalysis.total[0].hour.total.report.length -
                  vm.main.periodAnalysis.total[0].hour.total.pdf.spare;
                vm.main.periodAnalysis.total[0].hour.total.pdf.end = Math.floor(
                  length / vm.main.periodAnalysis.total[0].hour.total.pdf.d
                );
