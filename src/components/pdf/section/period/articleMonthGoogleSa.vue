<template>
  <div :class="getUniqueId">
    <template v-for="(index, i) in main.periodAnalysis.pages[3].google.month.pdf.end" :key="index + i + '-' + getUuidv4()">
      <div  class="pdf-a4-l periodAnalysis">
        ...
        <section class="d-flex pdf-card pdf-card-2 google">
          <div v-show="index === 1" class="row justify-content-between" ref="graphs">
            <div class="col crossGraphPeriodOnemonth d-flex justify-content-center">
              <div id="crossGraphPeriodOnemonth" class="crossGraphPeriodOnemonth" ></div>
            </div>
            <div class="col crossGraphPeriodTwomonth d-flex justify-content-center">
              <div id="crossGraphPeriodTwomonth" class="crossGraphPeriodTwomonth" ></div>
            </div>
          </div>
          <div class="report">
            <table class="report">
              <thead>
                <tr>
                  <td class="text-center" width="25%">월별</td>
              ...
              <tbody v-if="main.periodAnalysis.responseData !== null">
                <tr v-for="(data, index2) in getDatasForPeriod('google', 3, 'month', index)" :key="index2 + '-' + getUuidv4()" >
  ...
  methods: {
    ...
    getReport() {
      ...
      if (vm.report.activePage.periodAnalysis) {
        ...
        $.when(periodreportForMonth)
        .done(function(periodreportForMonth) {
          if(periodreportForMonth.result === "success") {
            var isData = false;
            vm.main.medias.forEach(function (media, index) {
              if (vm.report.activePage.periodAnalysis && vm.report.activeMedia[media]) {
                isData = true;
                vm.main.periodAnalysis.pages[index][media].month.responseData = periodreportForMonth;
                var media = media;
                var type = media === "kakaomo" ? "dsp" : "sa";
                let values = { cst: [], cv: [], clk: [], im: [], times: [] };
                vm.main.periodAnalysis.pages[index][media].month.report = [];
                var graphDatas;
                var totalDatas;

                totalDatas = periodreportForMonth.data.media[media];
                Object.keys(totalDatas).forEach(function (date, index2) {
                  ...
                  vm.main.periodAnalysis.pages[index][media].month.report.push(obj);
                });

                var length = vm.main.periodAnalysis.pages[index][media].month.report.length - vm.main.periodAnalysis.pages[index][media].month.pdf.spare;
                vm.main.periodAnalysis.pages[index][media].month.pdf.end = Math.floor( length / vm.main.periodAnalysis.pages[index][media].month.pdf.d );
                var n = length % vm.main.periodAnalysis.pages[index][media].month.pdf.d;

                if (vm.main.periodAnalysis.pages[index][media].month.pdf.end <= 0)
                  vm.main.periodAnalysis.pages[index][media].month.pdf.end += 1;
                else if (n > 0)
                  vm.main.periodAnalysis.pages[index][media].month.pdf.end += 1;

                vm.main.periodAnalysis.pages[index][media].month.pdf.end += 1; //12개용 페이지 추가

                Object.keys(vm.main.periodAnalysis.pages[index][media].month.graph).forEach(function (graph, index2) {
                  if (vm.main.periodAnalysis.pages[index][media].month.graph[graph])
                    vm.main.periodAnalysis.pages[index][media].month.graph[graph].dispose();

                  vm.$nextTick(function () {
                    if(document.querySelector(".periodAnalysis " + " ." + media + " #" + graph)) {
                      vm.main.periodAnalysis.pages[index][media].month.graph[graph] = echarts.init(document.querySelector(".periodAnalysis " + " ." + media + " #" + graph));
                      var option = {
                      ...
                      vm.main.periodAnalysis.pages[index][media].month.graph[graph].setOption(option);
                      vm.main.periodAnalysis.pages[index][media].month.graph[graph].on("finished",function () { vm.$emit("finished", { graph: graph + media, status: true });});
                    }
            ...
            if (isData) {
              vm.main.periodAnalysis.total[2].month.total.report = [];
              if (vm.isOneMedia()) {
                vm.$emit('rendering', {graph: 'crossGraphPeriodTotalOnemonth', status: true});
                vm.$emit('rendering', {graph: 'crossGraphPeriodTotalTwomonth', status: true});
                // return true;
              } else {
                ...
                vm.main.medias.forEach(function (media, index) {
                  if (vm.report.activePage.periodAnalysis && vm.report.activeMedia[media]) {
                    Object.keys(periodreportForMonth.data.media[media]).forEach(
                      function (month, index2) {
                        ["clk", "cr", "cst", "cv", "im"].forEach(function ( kpi, index3 ) {
                          var o = periodreportForMonth.data.media[media][month];
                          if (!obj[kpi][month]) obj[kpi][month] = [];

                          var v;
                          typeof o[kpi] === "string" ? (v = o[kpi].replace(",", "")) : (v = o[kpi]);
                          obj[kpi][month].push(v);
                        });
                      }
                    );
                  }
                ...
                Object.keys(obj).forEach(function (kpi, index) {
                  Object.keys(obj[kpi]).forEach(function (month, index) {
                    var month = month;
                    obj2[kpi][month] = vm.getSum(obj[kpi][month]);
                    values[kpi].push(obj2[kpi][month]);
                  });
                });
                ...
                ["ctr", "cpc", "cpa", "cvr", "roas"].forEach(function (kpi, index) {
                  Object.keys(periodreportForMonth.data.total).forEach(function (month,index2) {
                    if (month === "summary") return;

                    var month = month;
                    if (!obj2[kpi][month]) obj2[kpi][month] = {};
                    if (kpi === "ctr") { obj2[kpi][month] = obj2["clk"][month] > 0 && obj2["im"][month] > 0 ? ((obj2["clk"][month] / obj2["im"][month]) * 100).toFixed( 2 ) : 0; }
                    if (kpi === "cpc") { obj2[kpi][month] = obj2["cst"][month] > 0 && obj2["clk"][month] > 0 ? (obj2["cst"][month] / obj2["clk"][month]).toFixed(0) : 0; }
                    if (kpi === "cpa") { obj2[kpi][month] = obj2["cst"][month] > 0 && obj2["cv"][month] > 0 ? (obj2["cst"][month] / obj2["cv"][month]).toFixed(0) : 0; }
                    if (kpi === "cvr") { obj2[kpi][month] = obj2["cv"][month] > 0 && obj2["clk"][month] > 0 ? ((obj2["cv"][month] / obj2["clk"][month]) * 100).toFixed( 2 ) : 0; }
                    if (kpi === "roas") { obj2[kpi][month] = obj2["cr"][month] > 0 && obj2["cst"][month] > 0 ? ((obj2["cr"][month] / obj2["cst"][month]) * 100).toFixed( 0 ) : 0; }
                    values[kpi].push(obj2[kpi][month]);
                  });
                });
                ...
                var index = 0;
                // console.log(values);
                Object.keys(periodreportForMonth.data.total).forEach(function (month, i) {
                  if (month === "summary") return;
                  vm.main.periodAnalysis.total[2].month.total.report.push({
                    clk: values["clk"][index],
                    cpa: values["cpa"][index],
                    cpc: values["cpc"][index],
                    cr: values["cr"][index],
                    cst: values["cst"][index],
                    ctr: values["ctr"][index],
                    cv: values["cv"][index],
                    cvr: values["cvr"][index],
                    im: values["im"][index],
                    roas: values["roas"][index],
                    time: month,
                  });
                  periods.push(month);
                  index = index + 1;
                ...
                var length = vm.main.periodAnalysis.total[2].month.total.report.length - vm.main.periodAnalysis.total[2].month.total.pdf.spare;
                vm.main.periodAnalysis.total[2].month.total.pdf.end = Math.floor( length / vm.main.periodAnalysis.total[2].month.total.pdf.d );
                var n = length % vm.main.periodAnalysis.total[2].month.total.pdf.d;

                if (vm.main.periodAnalysis.total[2].month.total.pdf.end <= 0)
                  vm.main.periodAnalysis.total[2].month.total.pdf.end += 1;
                else if (n > 0) vm.main.periodAnalysis.total[2].month.total.pdf.end += 1;

                vm.main.periodAnalysis.total[2].month.total.pdf.end += 1; //12개용 페이지 추가

                Object.keys(vm.main.periodAnalysis.total[2].month.total.graph).forEach(
                  function (graph, index2) {
                    if (vm.main.periodAnalysis.total[2].month.total.graph[graph])
                      vm.main.periodAnalysis.total[2].month.total.graph[graph].dispose();

                    vm.$nextTick(function () {
                      if(document.querySelector(".periodAnalysis " + " ." + "naver" + " #" + graph)) {
                        vm.main.periodAnalysis.total[2].month.total.graph[graph] = echarts.init(document.querySelector(".periodAnalysis " + " ." + "naver" + " #" + graph));
                        var option = { 
                          color: ["#1795F0", "#6664DA", "#4EDF9F", "#4DC6DF"],
                          tooltip: { trigger: "axis", axisPointer: { type: "cross", crossStyle: { color: "#999" }, }, },
                          toolbox: { feature: {} },
                        ...
                        vm.main.periodAnalysis.total[2].month.total.graph[graph].setOption(option);
                        vm.main.periodAnalysis.total[2].month.total.graph[graph].on("finished", function () { vm.$emit("finished", { graph: graph, status: true });});
                      }
                    });
                  }
                );
