<template>
  <div :class="getUniqueId">
    <template v-for="(index, i) in main.periodAnalysis.pages[3].naverda.dayweek.pdf.end" :key="index + i + '-' + getUuidv4()">
      <div  class="pdf-a4-l periodAnalysis">
        <section class="d-flex pdf-card pdf-card-1">
          <div class="sub-title">
            <h2>기간별 분석 - GFA(요일별)</h2>
          </div>
        ...
        <section class="d-flex pdf-card pdf-card-2 naverda">
          <div v-show="index === 1" class="row justify-content-between" ref="graphs">
            <div class="col crossGraphPeriodOnedayweek d-flex justify-content-center">
              <div
                id="crossGraphPeriodOnedayweek"
                class="crossGraphPeriodOnedayweek"
              ></div>
            </div>
            <div class="col crossGraphPeriodTwodayweek d-flex justify-content-center">
              <div
                id="crossGraphPeriodTwodayweek"
                class="crossGraphPeriodTwodayweek"
              ></div>
            </div>
          </div>
          <div class="report">
            <table class="report">
              ...
              <tbody v-if="main.periodAnalysis.responseData !== null">
                <tr
                  v-for="(data, index2) in getDatasForPeriod('naverda', 3, 'dayweek', index)"
          ...
          if(periodreportForDayweek.result === "success") {
            ...
            vm.main.medias.forEach(function (media, index) {
              if (vm.report.activePage.periodAnalysis && vm.report.activeMedia[media]) {
                ...
                var length =
                  vm.main.periodAnalysis.pages[index][media].dayweek.report.length -
                  vm.main.periodAnalysis.pages[index][media].dayweek.pdf.spare;
                vm.main.periodAnalysis.pages[index][media].dayweek.pdf.end = Math.floor(
                  length / vm.main.periodAnalysis.pages[index][media].dayweek.pdf.d
                );
                ...
                Object.keys(
                  vm.main.periodAnalysis.pages[index][media].dayweek.graph
                ).forEach(function (graph, index2) {
              ...
              if (vm.isOneMedia()) {
                ...
                ["ctr", "cpc", "cpa", "cvr", "roas"].forEach(function (kpi, index) {
                  ["mon", "tue", "wed", "thu", "fri", "sat", "sun"].forEach(function (dayweek,index2) {
                    ...
                    if (kpi === "ctr") {
                          obj2[kpi][dayweek] =
                            obj2["clk"][dayweek] > 0 && obj2["im"][dayweek] > 0
                              ? (
                                  (obj2["clk"][dayweek] / obj2["im"][dayweek]) *
                                  100
                                ).toFixed(2)
                              : 0;
                    }
                    if (kpi === "cpc") {
                          obj2[kpi][dayweek] =
                            obj2["cst"][dayweek] > 0 && obj2["clk"][dayweek] > 0
                              ? (obj2["cst"][dayweek] / obj2["clk"][dayweek]).toFixed(0)
                              : 0;
                    }
                    if (kpi === "cpa") {
                          obj2[kpi][dayweek] =
                            obj2["cst"][dayweek] > 0 && obj2["cv"][dayweek] > 0
                              ? (obj2["cst"][dayweek] / obj2["cv"][dayweek]).toFixed(0)
                              : 0;
                    }
                    if (kpi === "cvr") {
                          obj2[kpi][dayweek] =
                            obj2["cv"][dayweek] > 0 && obj2["clk"][dayweek] > 0
                              ? (
                                  (obj2["cv"][dayweek] / obj2["clk"][dayweek]) *
                                  100
                                ).toFixed(2)
                              : 0;
                    }
                    if (kpi === "roas") {
                          obj2[kpi][dayweek] =
                            obj2["cr"][dayweek] > 0 && obj2["cst"][dayweek] > 0
                              ? (
                                  (obj2["cr"][dayweek] / obj2["cst"][dayweek]) *
                                  100
                                ).toFixed(0)
                              : 0;
                    }
