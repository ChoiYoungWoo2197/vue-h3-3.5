<template>
  <div :class="getUniqueId">
    <template v-for="(index, i) in main.periodAnalysis.pages[3].naverda.month.pdf.end" :key="index + i + '-' + getUuidv4()">
      <div  class="pdf-a4-l periodAnalysis">
        ...
        <section class="d-flex pdf-card pdf-card-2 naverda">
          <div v-show="index === 1" class="row justify-content-between" ref="graphs">
            <div class="col crossGraphPeriodOnemonth d-flex justify-content-center">
              <div
                id="crossGraphPeriodOnemonth"
                class="crossGraphPeriodOnemonth"
              ></div>
            </div>
            <div class="col crossGraphPeriodTwomonth d-flex justify-content-center">
              <div
                id="crossGraphPeriodTwomonth"
                class="crossGraphPeriodTwomonth"
              ></div>
            </div>
          </div>
          <div class="report">
            <table class="report">
              ...
              <tbody v-if="main.periodAnalysis.responseData !== null">
                <tr
                  v-for="(data, index2) in getDatasForPeriod('naverda', 3, 'month', index)"
...
export default {
  ...
  data() {
    return {
      ...
      main: {
        ...
        start: 0, //현재 페이지
        display: 20, //게시글의 수 
        medias: ["naver", "kakaosa", "kakaomo", "naverda"],        
      },
      pdf: {
  ...
  methods: {
    ...
    sortHour(data, fromDate, toDate) {
      ...
      ["total", "kakaomo", "naverda", "kakaosa", "naver"].forEach(function (currentMedia, index) {
          ...
          if(periodreportForMonth.result === "success") {
            ...
            vm.main.medias.forEach(function (media, index) {
              if (vm.report.activePage.periodAnalysis && vm.report.activeMedia[media]) {
                isData = true;
                vm.main.periodAnalysis.pages[index][media].month.responseData =
                  periodreportForMonth;
                var media = media;
                ...
                var length =
                  vm.main.periodAnalysis.pages[index][media].month.report.length -
                  vm.main.periodAnalysis.pages[index][media].month.pdf.spare;
                vm.main.periodAnalysis.pages[index][media].month.pdf.end = Math.floor(
                  length / vm.main.periodAnalysis.pages[index][media].month.pdf.d
                );
              ...
              if (vm.isOneMedia()) {
                  ...
                  if (vm.report.activePage.periodAnalysis && vm.report.activeMedia[media]) {
                    Object.keys(periodreportForMonth.data.media[media]).forEach(
                      function (month, index2) {
                        ["clk", "cr", "cst", "cv", "im"].forEach(function (
                          kpi,
                          index3
                        ) {
                ...
                ["ctr", "cpc", "cpa", "cvr", "roas"].forEach(function (kpi, index) {
                  Object.keys(periodreportForMonth.data.total).forEach(function (month,index2) {
                    ...
                    if (kpi === "ctr") {
                      obj2[kpi][month] =
                        obj2["clk"][month] > 0 && obj2["im"][month] > 0
                          ? ((obj2["clk"][month] / obj2["im"][month]) * 100).toFixed(
                              2
                            )
                          : 0;
                    }
                    if (kpi === "cpc") {
                      obj2[kpi][month] =
                        obj2["cst"][month] > 0 && obj2["clk"][month] > 0
                          ? (obj2["cst"][month] / obj2["clk"][month]).toFixed(0)
                          : 0;
                    }
                    if (kpi === "cpa") {
                      obj2[kpi][month] =
                        obj2["cst"][month] > 0 && obj2["cv"][month] > 0
                          ? (obj2["cst"][month] / obj2["cv"][month]).toFixed(0)
                          : 0;
                    }
                    if (kpi === "cvr") {
                      obj2[kpi][month] =
                        obj2["cv"][month] > 0 && obj2["clk"][month] > 0
                          ? ((obj2["cv"][month] / obj2["clk"][month]) * 100).toFixed(
                              2
                            )
                          : 0;
                    }
                    if (kpi === "roas") {
                      obj2[kpi][month] =
                        obj2["cr"][month] > 0 && obj2["cst"][month] > 0
                          ? ((obj2["cr"][month] / obj2["cst"][month]) * 100).toFixed(
                              0
                            )
                          : 0;
                    }
                ...
                var length =
                  vm.main.periodAnalysis.total[2].month.total.report.length -
                  vm.main.periodAnalysis.total[2].month.total.pdf.spare;
                vm.main.periodAnalysis.total[2].month.total.pdf.end = Math.floor(
                  length / vm.main.periodAnalysis.total[2].month.total.pdf.d
                );
