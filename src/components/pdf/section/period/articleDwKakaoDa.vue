<template>
  <div :class="getUniqueId">
    <template v-for="(index, i) in main.periodAnalysis.pages[2].kakaomo.dayweek.pdf.end" :key="index + i + '-' + getUuidv4()">
      <div  class="pdf-a4-l periodAnalysis">
        <section class="d-flex pdf-card pdf-card-1">
          <div class="sub-title">
            <h2>기간별 분석 - 카카오 모먼트(요일별)</h2>
          </div>
          ...
          <div class="report">
            <table class="report">
              ...
              <tbody v-if="main.periodAnalysis.responseData !== null">
                <tr
                  v-for="(data, index2) in getDatasForPeriod('kakaomo', 2, 'dayweek', index)"
