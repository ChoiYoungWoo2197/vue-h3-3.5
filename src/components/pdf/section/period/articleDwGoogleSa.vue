<template>
  <div :class="getUniqueId">
    <template v-for="(index, i) in main.periodAnalysis.pages[3].google.dayweek.pdf.end" :key="index + i + '-' + getUuidv4()">
        ...
        <section class="d-flex pdf-card pdf-card-2 google">
          ...
          <div class="report">
            <table class="report">
              <thead>
                <tr>
                  <td class="text-center" width="25%">요일별</td>
              ...
              <tbody v-if="main.periodAnalysis.responseData !== null">
                <tr v-for="(data, index2) in getDatasForPeriod('google', 3, 'dayweek', index)" :key="index2 + '-' + getUuidv4()" >
  ...
  methods: {
    ...
    getReport() {
      ...
      if (vm.report.activePage.periodAnalysis) {
        var periodreportForDayweek = $.ajax({
          type: "get",
        ...
        $.when(periodreportForDayweek)
        .done(function(periodreportForDayweek) {
          if(periodreportForDayweek.result === "success") {
            var isData = false;
            vm.main.medias.forEach(function (media, index) {
              if (vm.report.activePage.periodAnalysis && vm.report.activeMedia[media]) {
                isData = true;
                vm.main.periodAnalysis.pages[index][media].dayweek.responseData = periodreportForDayweek;
                var media = media;
                var type = media === "kakaomo" ? "dsp" : "sa";
                let values = { cst: [], cv: [], clk: [], im: [], times: [] };
                vm.main.periodAnalysis.pages[index][media].dayweek.report = [];
                var graphDatas;
                var totalDatas;
                var dayweek = { 0: "월요일", 1: "화요일", 2: "수요일", 3: "목요일", 4: "금요일", 5: "토요일", 6: "일요일", };

                totalDatas = sortWeekDay(periodreportForDayweek.data.media[media]);
                Object.keys(totalDatas).forEach(function (date, index2) {
                  var obj = totalDatas[date];
                  obj.time = dayweek[index2];

                  values.times.push(dayweek[index2]);
                  values.cst.push(obj.cst);
                  values.cv.push(obj.cv);
                  values.clk.push(obj.clk);
                  values.im.push(obj.im);
                  vm.main.periodAnalysis.pages[index][media].dayweek.report.push(obj);
                });

                var length = vm.main.periodAnalysis.pages[index][media].dayweek.report.length - vm.main.periodAnalysis.pages[index][media].dayweek.pdf.spare;
                vm.main.periodAnalysis.pages[index][media].dayweek.pdf.end = Math.floor( length / vm.main.periodAnalysis.pages[index][media].dayweek.pdf.d );
                var n = length % vm.main.periodAnalysis.pages[index][media].dayweek.pdf.d;

                if (vm.main.periodAnalysis.pages[index][media].dayweek.pdf.end <= 0)
                  vm.main.periodAnalysis.pages[index][media].dayweek.pdf.end += 1;
                else if (n > 0)
                  vm.main.periodAnalysis.pages[index][media].dayweek.pdf.end += 1;

                vm.main.periodAnalysis.pages[index][media].dayweek.pdf.end += 1; //12개용 페이지 추가

                Object.keys( vm.main.periodAnalysis.pages[index][media].dayweek.graph ).forEach(function (graph, index2) {
                  if (vm.main.periodAnalysis.pages[index][media].dayweek.graph[graph])
                    vm.main.periodAnalysis.pages[index][media].dayweek.graph[graph].dispose();

                  vm.$nextTick(function () {
                    if(document.querySelector(".periodAnalysis " + " ." + media + " #" + graph)) {
                      vm.main.periodAnalysis.pages[index][media].dayweek.graph[graph] = echarts.init(document.querySelector(".periodAnalysis " + " ." + media + " #" + graph));
                      var option = {
                      ...
                      vm.main.periodAnalysis.pages[index][media].dayweek.graph[graph].setOption(option);
                      vm.main.periodAnalysis.pages[index][media].dayweek.graph[graph].on("finished", function () { vm.$emit("finished", {graph: graph + media, status: true}); });
                    }
            ...
            if (isData) {
              vm.main.periodAnalysis.total[1].dayweek.total.report = [];
              if (vm.isOneMedia()) {
                vm.$emit('rendering', {graph: 'crossGraphPeriodTotalOnedayweek', status: true});
                vm.$emit('rendering', {graph: 'crossGraphPeriodTotalTwodayweek', status: true});
                // return true;
              } else {
                ...
                vm.main.medias.forEach(function (media, index) {
                  if (vm.report.activePage.periodAnalysis && vm.report.activeMedia[media]) {
                    Object.keys(periodreportForDayweek.data.media[media]).forEach(
                      function (dayweek, index2) {
                        ["clk", "cr", "cst", "cv", "im"].forEach(function (kpi,index3) {
                          var o = periodreportForDayweek.data.media[media][dayweek];
                          if (!obj[kpi][dayweek]) obj[kpi][dayweek] = [];

                          var v;
                          typeof o[kpi] === "string" ? (v = o[kpi].replace(",", "")) : (v = o[kpi]);
                          obj[kpi][dayweek].push(v);
                        });
                      }
                    );
                  }
                ...
                Object.keys(obj).forEach(function (kpi, index) {
                  ["mon", "tue", "wed", "thu", "fri", "sat", "sun"].forEach(function (dayweek, index) {
                    var dayweek = dayweek;
                    obj2[kpi][dayweek] = vm.getSum(obj[kpi][dayweek]);
                    values[kpi].push(obj2[kpi][dayweek]);
                  });
                });

                ["ctr", "cpc", "cpa", "cvr", "roas"].forEach(function (kpi, index) {
                  ["mon", "tue", "wed", "thu", "fri", "sat", "sun"].forEach(function (dayweek,index2) {
                    var dayweek = dayweek;
                    if (!obj2[kpi][dayweek]) obj2[kpi][dayweek] = {};
                    if (kpi === "ctr") { obj2[kpi][dayweek] = obj2["clk"][dayweek] > 0 && obj2["im"][dayweek] > 0 ? ( (obj2["clk"][dayweek] / obj2["im"][dayweek]) * 100 ).toFixed(2) : 0; }
                    if (kpi === "cpc") { obj2[kpi][dayweek] = obj2["cst"][dayweek] > 0 && obj2["clk"][dayweek] > 0 ? (obj2["cst"][dayweek] / obj2["clk"][dayweek]).toFixed(0) : 0; }
                    if (kpi === "cpa") { obj2[kpi][dayweek] = obj2["cst"][dayweek] > 0 && obj2["cv"][dayweek] > 0 ? (obj2["cst"][dayweek] / obj2["cv"][dayweek]).toFixed(0) : 0; }
                    if (kpi === "cvr") { obj2[kpi][dayweek] = obj2["cv"][dayweek] > 0 && obj2["clk"][dayweek] > 0 ? ( (obj2["cv"][dayweek] / obj2["clk"][dayweek]) * 100 ).toFixed(2) : 0; }
                    if (kpi === "roas") { obj2[kpi][dayweek] = obj2["cr"][dayweek] > 0 && obj2["cst"][dayweek] > 0 ? ( (obj2["cr"][dayweek] / obj2["cst"][dayweek]) * 100 ).toFixed(0) : 0; }
                    values[kpi].push(obj2[kpi][dayweek]);
                  });
                });
                ...
                var length = vm.main.periodAnalysis.total[1].dayweek.total.report.length - vm.main.periodAnalysis.total[1].dayweek.total.pdf.spare;
                vm.main.periodAnalysis.total[1].dayweek.total.pdf.end = Math.floor( length / vm.main.periodAnalysis.total[1].dayweek.total.pdf.d );
                ...
                Object.keys(vm.main.periodAnalysis.total[1].dayweek.total.graph).forEach( function (graph, index2) {
