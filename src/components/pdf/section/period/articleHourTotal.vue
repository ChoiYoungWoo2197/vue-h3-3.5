<template>
  <div :class="getUniqueId">
    <template v-for="(index, i) in main.periodAnalysis.total[0].hour.total.pdf.end" :key="index + i + '-' + getUuidv4()">
      <div  class="pdf-a4-l periodAnalysis">
        ...
        <section class="d-flex pdf-card pdf-card-2 naver">
          <div v-show="index === 1" class="row justify-content-between">
            <div class="col crossGraphPeriodTotalOnehour d-flex justify-content-center">
              <div id="crossGraphPeriodTotalOnehour" class="crossGraphPeriodTotalOnehour" ></div>
            </div>
            <div class="col crossGraphPeriodTotalTwohour d-flex justify-content-center">
              <div id="crossGraphPeriodTotalTwohour" class="crossGraphPeriodTotalTwohour" ></div>
            </div>
          </div>
          <div class="report">
            <table class="report">
              ...
              <tbody v-if="main.periodAnalysis.responseData !== null">
                <tr v-for="(data, index2) in getDatasForPeriodTotal(0, 'hour', index)" :key="index2 + '-' + getUuidv4()" >
...
export default {
  ...
  methods: {
    ...
    setPeriodAnalysis() {
      ...
      vm.main.periodAnalysis.pages[3].naverda.responseData = {};
      vm.main.periodAnalysis.pages[4].google.responseData = {};
    },
    sortHour(data, fromDate, toDate) {
      var vm = this;
      var sorted = {
        media: { kakaomo: {}, naverda: {}, kakaosa: {}, naver: {}, google: {}, },
        total: { summary: {}, },
      };

      ["total", "kakaomo", "naverda", "kakaosa", "naver", "google"].forEach(function (currentMedia, index) {
      ...
      if (vm.report.activePage.periodAnalysis) {
        ...
        .done(function(periodreportForHour) {
          if(periodreportForHour.result === "success") {
            var isData = false;
            vm.main.periodAnalysis.pages[0].naver.hour.responseData = periodreportForHour;
            vm.main.medias.forEach(function (media, index) {
              if (
                ...
                var graphDatas;
                var totalDatas = { media: { kakaomo: {}, naverda:{}, kakaosa: {}, naver: {}, google: {} }, total: { summary: {} }, };
                ...
                Object.keys(vm.main.periodAnalysis.pages[index][media].hour.graph).forEach(
                  function (graph, index2) {
                    // console.log(vm.periodAnalysis.pages[index][media].hour.graph, graph);
