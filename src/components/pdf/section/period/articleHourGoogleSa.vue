<template>
  <div :class="getUniqueId">
    <template v-for="(index, i) in main.periodAnalysis.pages[3].google.hour.pdf.end" :key="index + i + '-' + getUuidv4()">
      <div  class="pdf-a4-l periodAnalysis">
        ...
        <section class="d-flex pdf-card pdf-card-2 google">
          <div v-show="index === 1" class="row justify-content-between" ref="graphs">
            <div class="col crossGraphPeriodOnehour d-flex justify-content-center">
              <div id="crossGraphPeriodOnehour" class="crossGraphPeriodOnehour" ></div>
            </div>
            <div class="col crossGraphPeriodTwohour d-flex justify-content-center">
              <div id="crossGraphPeriodTwohour" class="crossGraphPeriodTwohour" ></div>
            </div>
          </div>
          <div class="report">
            <table class="report">
              <thead>
                <tr>
                  <td class="text-center" width="25%">시간별</td>
              ...
              <tbody v-if="main.periodAnalysis.responseData !== null">
                <tr v-for="(data, index2) in getDatasForPeriod('google', 3, 'hour', index)" :key="index2 + '-' + getUuidv4()" >
                  <td class="text-left">{{ data.time + "시" }}</td>
                  <template v-for="(k,i) in Object.keys(sortDraggable())">
...
export default {
    ...
    return {
      ...
      main: {
        periodAnalysis: {
          ...
          activeKpi: null,
          activeMedia: null,
          activeBtn: { hour: true, dayweek: false, day: false, month: false, period: "", periodUnit: "hour", },
          datas: [],
        },  
        start: 0, //현재 페이지
        display: 20, //게시글의 수 
  ...
  methods: {
    ...
    setPeriodAnalysis() {
      ...
      vm.main.periodAnalysis.pages[3].google.responseData = {};
    },
    sortHour(data, fromDate, toDate) {
      var vm = this;
      var sorted = {
        media: { kakaomo: {}, kakaosa: {}, naver: {}, google: {} },
        total: { summary: {}, },
      };

      ["total", "kakaomo", "kakaosa", "naver", "google"].forEach(function (currentMedia, index) {
    ...
    getDatasForPeriod(media, arrindex, time, index) {
      ...
      if (index === 1) {
        ...
        start = index - 2 > 0 ? vm.main.periodAnalysis.pages[arrindex][media][time].pdf.spare + vm.main.periodAnalysis.pages[arrindex][media][time].pdf.d * (index - 2) : vm.main.periodAnalysis.pages[arrindex][media][time].pdf.spare;
      }

      for (let i = 0; i < count; i++) {
        if (!vm.main.periodAnalysis.pages[arrindex][media][time].report) continue;
        if (!vm.main.periodAnalysis.pages[arrindex][media][time].report.at(start + i)) continue;
    ...
    getDatasForPeriodTotal(arrindex, time, index) {
      ...
      if (index === 1) {
        ...
        start = index - 2 > 0 ? vm.main.periodAnalysis.total[arrindex][time].total.pdf.spare + vm.main.periodAnalysis.total[arrindex][time].total.pdf.d * (index - 2) : vm.main.periodAnalysis.total[arrindex][time].total.pdf.spare;
      }
    ...
    getReport() {
      ...
      if (vm.report.activePage.periodAnalysis) {
        ...
        $.when(periodreportForHour)
        .done(function(periodreportForHour) {
          if(periodreportForHour.result === "success") {
            var isData = false;
            vm.main.periodAnalysis.pages[3].google.hour.responseData = periodreportForHour;
            vm.main.medias.forEach(function (media, index) {
              if (
                ...
                vm.main.periodAnalysis.pages[index][media].hour.report = [];
                var graphDatas;
                var totalDatas = { media: { kakaomo: {}, kakaosa: {}, naver: {}, google: {} }, total: { summary: {} }, };
                totalDatas = vm.sortHour(periodreportForHour.data,vm.report.reportInfo.start,vm.report.reportInfo.end);
                Object.keys(totalDatas.media[media]).sort().forEach(function (date, index2) {
                    var obj = totalDatas.media[media][date];
                    obj.time = index2;

                    values.times.push(index2);
                    values.cst.push(obj.cst);
                    values.cv.push(obj.cv);
                    values.clk.push(obj.clk);
                    values.im.push(obj.im);
                    vm.main.periodAnalysis.pages[index][media].hour.report.push(obj);
                  });

                var length = vm.main.periodAnalysis.pages[index][media].hour.report.length - vm.main.periodAnalysis.pages[index][media].hour.pdf.spare;
                vm.main.periodAnalysis.pages[index][media].hour.pdf.end = Math.floor( length / vm.main.periodAnalysis.pages[index][media].hour.pdf.d );
                var n = length % vm.main.periodAnalysis.pages[index][media].hour.pdf.d;

                if (vm.main.periodAnalysis.pages[index][media].hour.pdf.end <= 0)
                  vm.main.periodAnalysis.pages[index][media].hour.pdf.end += 1;
                else if (n > 0)
                  vm.main.periodAnalysis.pages[index][media].hour.pdf.end += 1;

                vm.main.periodAnalysis.pages[index][media].hour.pdf.end += 1; //12개용 페이지 추가

                Object.keys(vm.main.periodAnalysis.pages[index][media].hour.graph).forEach(
                  function (graph, index2) {
                    if (vm.main.periodAnalysis.pages[index][media].hour.graph[graph])
                      vm.main.periodAnalysis.pages[index][media].hour.graph[graph].dispose();

                    vm.$nextTick(function () {
                      if(document.querySelector(".periodAnalysis " + " ." + media + " #" + graph)) {
                        vm.main.periodAnalysis.pages[index][media].hour.graph[graph] = echarts.init(document.querySelector(".periodAnalysis " + " ." + media + " #" + graph));
                        var option = {
                          ...
                          xAxis: [ { type: "category", data: values.times, axisPointer: { type: "shadow" }, axisLabel: { fontSize: "12" }, }, ],
                          yAxis: [
                        ...
                        vm.main.periodAnalysis.pages[index][media].hour.graph[graph].setOption(option);
                        vm.main.periodAnalysis.pages[index][media].hour.graph[graph].on("finished", function () {vm.$emit("finished", {graph: graph + media, status: true});}
                        );
                      }
            ...
            if (isData) {
              vm.main.periodAnalysis.total[0].hour.total.report = [];
              if (vm.isOneMedia()) {
                vm.$emit('rendering', {graph: 'crossGraphPeriodTotalOnehour', status: true});
                vm.$emit('rendering', {graph: 'crossGraphPeriodTotalTwohour', status: true});
                // return true;
              } else {
                ...
                vm.main.medias.forEach(function (media, index) {
                  if (vm.report.activePage.periodAnalysis && vm.report.activeMedia[media]) {
                    Object.keys(periodreportForHour.data.media[media]).forEach(
                      function (date, index2) {
                        Object.keys(periodreportForHour.data.media[media][date]).forEach(function (hour, index3) {
                          ["clk", "cr", "cst", "cv", "im"].forEach(function (kpi, index4) {
                            var o =  periodreportForHour.data.media[media][date][hour];
                            if (!obj[kpi][hour]) obj[kpi][hour] = [];
                ...
                Object.keys(obj).forEach(function (kpi, index) {
                  for (i = 0; i <= 23; i++) {
                    var hour = i < 10 ? "0" + i : i;
                    obj2[kpi][hour] = vm.getSum(obj[kpi][hour]);
                    values[kpi].push(obj2[kpi][hour]);
                  }
                });

                ["ctr", "cpc", "cpa", "cvr", "roas"].forEach(function (kpi, index2) {
                  for (i = 0; i <= 23; i++) {
                    var hour = i < 10 ? "0" + i : i;
                    if (!obj2[kpi][hour]) obj2[kpi][hour] = {};
                    if (kpi === "ctr") { obj2[kpi][hour] = obj2["clk"][hour] > 0 && obj2["im"][hour] > 0 ? ((obj2["clk"][hour] / obj2["im"][hour]) * 100).toFixed(2) : 0; }
                    if (kpi === "cpc") { obj2[kpi][hour] = obj2["cst"][hour] > 0 && obj2["clk"][hour] > 0 ? (obj2["cst"][hour] / obj2["clk"][hour]).toFixed(0) : 0; }
                    if (kpi === "cpa") { obj2[kpi][hour] = obj2["cst"][hour] > 0 && obj2["cv"][hour] > 0 ? (obj2["cst"][hour] / obj2["cv"][hour]).toFixed(0) : 0; }
                    if (kpi === "cvr") { obj2[kpi][hour] = obj2["cv"][hour] > 0 && obj2["clk"][hour] > 0 ? ((obj2["cv"][hour] / obj2["clk"][hour]) * 100).toFixed(2) : 0; }
                    if (kpi === "roas") { obj2[kpi][hour] = obj2["cr"][hour] > 0 && obj2["cst"][hour] > 0 ? ((obj2["cr"][hour] / obj2["cst"][hour]) * 100).toFixed(0) : 0; }
                    values[kpi].push(obj2[kpi][hour]);
                  }
                });
                ...
                let periods = [];
                for (var i = 0; i <= 23; i++) {
                  vm.main.periodAnalysis.total[0].hour.total.report.push({
                    clk: values["clk"][i],
                    cpa: values["cpa"][i],
                    cpc: values["cpc"][i],
                    cr: values["cr"][i],
                ...
                var length = vm.main.periodAnalysis.total[0].hour.total.report.length - vm.main.periodAnalysis.total[0].hour.total.pdf.spare;
                vm.main.periodAnalysis.total[0].hour.total.pdf.end = Math.floor( length / vm.main.periodAnalysis.total[0].hour.total.pdf.d );
                var n = length % vm.main.periodAnalysis.total[0].hour.total.pdf.d;

                if (vm.main.periodAnalysis.total[0].hour.total.pdf.end <= 0)
                  vm.main.periodAnalysis.total[0].hour.total.pdf.end += 1;
                else if (n > 0) vm.main.periodAnalysis.total[0].hour.total.pdf.end += 1;

                vm.main.periodAnalysis.total[0].hour.total.pdf.end += 1; //12개용 페이지 추가

                Object.keys(vm.main.periodAnalysis.total[0].hour.total.graph).forEach(
                  function (graph, index2) {
                    if (vm.main.periodAnalysis.total[0].hour.total.graph[graph])
                      vm.main.periodAnalysis.total[0].hour.total.graph[graph].dispose();
                    
                    vm.$nextTick(function () {
                      if(document.querySelector(".periodAnalysis " + " ." + "naver" + " #" + graph)) {
                        vm.main.periodAnalysis.total[0].hour.total.graph[graph] = echarts.init(document.querySelector(".periodAnalysis " + " ." + "naver" + " #" + graph));
                        var option = {
                          color: ["#1795F0", "#6664DA", "#4EDF9F", "#4DC6DF"],
                          tooltip: { trigger: "axis", axisPointer: { type: "cross", crossStyle: { color: "#999" }, }, },
                          toolbox: { feature: {} },
                          ...
                          xAxis: [
                            { type: "category", data: periods, axisPointer: { type: "shadow" }, axisLabel: { fontSize: "12" }, },
                          ],
                          yAxis: [
                            { type: "value", name: !index2 ? "노출수" : "광고비", axisLabel: { formatter: "{value}" }, axisLabel: { fontSize: "12" }, },
                            { type: "value", name: !index2 ? "클릭수" : "전환수", axisLabel: { formatter: "{value}" }, axisLabel: { fontSize: "12" }, },
                        ...
                        vm.main.periodAnalysis.total[0].hour.total.graph[graph].setOption(option);
                        vm.main.periodAnalysis.total[0].hour.total.graph[graph].on("finished", function () {vm.$emit("finished", {graph: graph, status: true});});
                      }
