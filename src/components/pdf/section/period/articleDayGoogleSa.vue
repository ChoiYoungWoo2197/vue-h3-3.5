<template>
  <div :class="getUniqueId">
    <template v-for="(index, i) in main.periodAnalysis.pages[3].google.day.pdf.end" :key="index + i + '-' + getUuidv4()">
        ...
        <section class="d-flex pdf-card pdf-card-2 google">
          <div v-show="index === 1" class="row justify-content-between" ref="graphs">
            <div class="col crossGraphPeriodOneday d-flex justify-content-center">
              <div id="crossGraphPeriodOneday" class="crossGraphPeriodOneday" ></div>
            </div>
            <div class="col crossGraphPeriodTwoday d-flex justify-content-center">
              <div id="crossGraphPeriodTwoday" class="crossGraphPeriodTwoday" ></div>
            </div>
          </div>
          <div class="report">
            <table class="report">
              ...
              <tbody v-if="main.periodAnalysis.responseData !== null">
                <tr v-for="(data, index2) in getDatasForPeriod('google', 3, 'day', index)" :key="index2 + '-' + getUuidv4()" >
          ...
          if(periodreportForDay.result === "success") {
            ...
            vm.main.medias.forEach(function (media, index) {
              if ( vm.report.activePage.periodAnalysis && vm.report.activeMedia[media] ) {
                isData = true;
                vm.main.periodAnalysis.pages[index][media].day.responseData = periodreportForDay;
                var media = media;
                ...
                var length = vm.main.periodAnalysis.pages[index][media].day.report.length - vm.main.periodAnalysis.pages[index][media].day.pdf.spare;
                vm.main.periodAnalysis.pages[index][media].day.pdf.end = Math.floor( length / vm.main.periodAnalysis.pages[index][media].day.pdf.d );
            ...
            if (isData) {
              ...
              if (vm.isOneMedia()) {
                ...
                ["ctr", "cpc", "cpa", "cvr", "roas"].forEach(function (kpi, index) {
                  for (var i = 0; i <= diff; i++) {
                    var date = moment(vm.report.reportInfo.start).add(i, "days").format("YYYY-MM-DD");

                    if (!obj2[kpi][date]) obj2[kpi][date] = {};
                    if (kpi === "ctr") { obj2[kpi][date] = obj2["clk"][date] > 0 && obj2["im"][date] > 0 ? ((obj2["clk"][date] / obj2["im"][date]) * 100).toFixed(2) : 0; }
                    if (kpi === "cpc") { obj2[kpi][date] = obj2["cst"][date] > 0 && obj2["clk"][date] > 0 ? (obj2["cst"][date] / obj2["clk"][date]).toFixed(0) : 0; }
                    if (kpi === "cpa") { obj2[kpi][date] = obj2["cst"][date] > 0 && obj2["cv"][date] > 0 ? (obj2["cst"][date] / obj2["cv"][date]).toFixed(0) : 0; }
                    if (kpi === "cvr") { obj2[kpi][date] = obj2["cv"][date] > 0 && obj2["clk"][date] > 0 ? ((obj2["cv"][date] / obj2["clk"][date]) * 100).toFixed(2) : 0; }
                    if (kpi === "roas") { obj2[kpi][date] = obj2["cr"][date] > 0 && obj2["cst"][date] > 0 ? ((obj2["cr"][date] / obj2["cst"][date]) * 100).toFixed(0) : 0; }
                    values[kpi].push(obj2[kpi][date]);
                  }
                });
                ...
                var length = vm.main.periodAnalysis.total[3].day.total.report.length - vm.main.periodAnalysis.total[3].day.total.pdf.spare;
                vm.main.periodAnalysis.total[3].day.total.pdf.end = Math.floor( length / vm.main.periodAnalysis.total[3].day.total.pdf.d );
