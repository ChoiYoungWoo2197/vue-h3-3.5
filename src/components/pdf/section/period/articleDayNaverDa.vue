<template>
  <div :class="getUniqueId">
    <template v-for="(index, i) in main.periodAnalysis.pages[3].naverda.day.pdf.end" :key="index + i + '-' + getUuidv4()">
          ...
          <div class="report">
            <table class="report">
              ...
              <tbody v-if="main.periodAnalysis.responseData !== null">
                <tr
                  v-for="(data, index2) in getDatasForPeriod('naverda', 3, 'day', index)"
...
export default {
  ...
  methods: {
    ...
    sortHour(data, fromDate, toDate) {
      ...
      ["total", "kakaomo", "kakaosa", "naver", "naverda"].forEach(function (currentMedia, index) {
