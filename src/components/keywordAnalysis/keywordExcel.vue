  ...
  methods: {
    clickExcel(o) {
      ...
      $.when(report, im, clk, cpc, cst).done(function(report, im, clk, cpc, cst) {
        setTimeout(function() {
          vm.exportExcel(o.md, report[0].data.keywords, im[0].data.kpis, clk[0].data.kpis, cpc[0].data.kpis, cst[0].data.kpis);
        }, 1000);        
      }).then(function(e) { vm.$emit('hideMask'); });
    },
    async exportExcel(media, report, im, clk, cpc, cst) {
      try {
        var response = await fetch('/keyword_template.xlsx');
        ...
        saveAs(blob, `keyword_report_${moment().format("YYYYMMDDHHmmss")}.xlsx`);
      } catch(e) {
        console.error(e);
      }      
    }
