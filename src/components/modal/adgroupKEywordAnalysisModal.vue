        ...
        <div class="modal-body">
          ...
          <article>
            ...
            <table class="report nshop">
              <thead>
                <tr>
                  <td class="text-left" width="30%">그룹</td>
                  <template v-for="(k,i) in ['im','clk','ctr','cpc','cst','cv','cr','cpa','cvr','roas']">
                    <td class="text-right" :key="i+getUuidv4()" v-if="kpi[k].check">
                      {{getKpiName(k)}}
                      ...
                      <template v-else><button class="active" @click="clickSort(k+'d')"><i class="bi bi-caret-down-fill"></i></button></template>
                    </td>
                  </template>
                </tr>
              </thead>
              <tbody>
                ...
                <template v-else>
                  <tr v-for="data in getDatasForMain" :key="data.adgroup_id + 'L'">
                    <td class="text-left">{{ data.adgroup_name }}</td>
                    <template v-for="(k,i) in ['im','clk','ctr','cpc','cst','cv','cr','cpa','cvr','roas']">
                      <td class="text-right" v-if="kpi[k].check" :key="i+getUuidv4()">{{priceToString(data[k]) + getUnit(k)}}</td>
                    </template>
                  </tr>
                </template>
              </tbody>
...
import { isMobile, priceToString, getMedia, getKpiName, uuidv4, getUnit } from "@/assets/js/common.js";
...
export default {
  ...
  methods: {
    ...
    showModal(campaignId, media, start, end) {
      var vm = this;
      vm.initForm();
      vm.media = media;
